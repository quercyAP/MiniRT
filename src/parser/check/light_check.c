...
/*   Updated: 2023/05/02 14:33:57 by tdechand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

void	light_check(t_pars_line pars_line)
{
	...
	if (pars_line.token_tab[2] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Light", pars_line.id));
	intensity_check(pars_line.d_pars, pars_line.token_tab[2], pars_line.id);
	if (pars_line.token_tab[3] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Light", pars_line.id));
	color_check(pars_line.d_pars, pars_line.token_tab[3], pars_line.id);
