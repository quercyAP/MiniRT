...
/*   Updated: 2023/05/02 14:34:54 by tdechand         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "minirt.h"

void	triangle_check(t_pars_line pars_line)
{
	if (pars_line.token_tab[1] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Triangle", pars_line.id));
	coord_check(pars_line.d_pars, pars_line.token_tab[1], pars_line.id);
	if (pars_line.token_tab[2] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Triangle", pars_line.id));
	coord_check(pars_line.d_pars, pars_line.token_tab[2], pars_line.id);
	if (pars_line.token_tab[3] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Triangle", pars_line.id));
	coord_check(pars_line.d_pars, pars_line.token_tab[3], pars_line.id);
	if (pars_line.token_tab[4] == NULL)
		return ((void)add_error(pars_line.d_pars, TOO_FEW_PARAM_ER,
				"Triangle", pars_line.id));
	color_check(pars_line.d_pars, pars_line.token_tab[4], pars_line.id);
